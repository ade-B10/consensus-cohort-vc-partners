...
#
# Usage:
#   1. Put the updated unencrypted file at source/Consensus_Cohort_VC_Partners.html
...
SOURCE_FILE="source/Consensus_Cohort_VC_Partners.html"
OUTPUT_FILE="index.html"
LABEL="Base10 · Consensus VC Partner Cohort {{SITE_NAME}} DNA"
INSTRUCTIONS="Enter the password to access Base10 · Consensus VC Partner Cohort {{SITE_NAME}} DNA."
REMEMBER_DAYS=14
...
if [ ! -f "$SOURCE_FILE" ]; then
  ...
  echo "  mkdir -p source"
  echo "  cp /Users/ade/Desktop/Urizen/Consensus_Cohort_VC_Partners.html source/"
  echo ""
  exit 1
fi
...
sed -i.bak "s|<title>Protected Page</title>|<title>Base10 · Consensus VC Partner Cohort {{SITE_NAME}} DNA</title>|" "$OUTPUT_FILE"
...
echo "  git commit -m \"Refresh Base10 · Consensus VC Partner Cohort {{SITE_NAME}} DNA ($(date +%Y-%m-%d))\""
echo "  git push"
echo ""
